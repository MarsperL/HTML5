window.onload = function () {
	...
	soundRemind = true;
	tick = false;
	soundTick = "time/media/audio_tick.mp3";
	soundMission = "time/media/audio_work.mp3";
	soundMissionDef = soundMission;
	soundRest = "time/media/audio_rest.mp3";
	...
	for (var i = 0; i < oRadiosUls.length; i++) {
 	...
 	for (var i = 0; i < oBaseLiInput.length; i++) {
 		(function (index) {
 			oBaseLiInput[index].onchange = function () {
 				switch(index) {
 					case 0 :
 						clockTime =60* oBaseLiInput[index].value;
 						break;
 					case 1 :
 						clockSRestTime =60* oBaseLiInput[index].value;
 						break;
 					case 2 :
 						clockLRestTime =60* oBaseLiInput[index].value;
 						break;
 					case 3 :
 						lRestInterval =60* oBaseLiInput[index].value;
 				}
 			}
 		})(i)
 	}
 	...
 	oBtn.onclick = function () {
 		...
 		var t = parseInt(clockTime/60) + parseInt(clockSRestTime/60);
 		liTotalTime.innerHTML = t + "分";
 		liMissionTime.innerHTML = clockTime/60 + "分";
 		liRestTime.innerHTML = clockSRestTime/60 + "分";
 	}
 	/* 任务界面添加任务按钮的点击事件 */
		...
		EventStart : function () {
			...
			this.oSpan2.onclick = function () {
				...
				if (!working) {  //如果不是在工作状态，即按钮显示为开始按钮
					if (innerText == "&#xe609;") {  //开始按钮
						this.getElementsByTagName("i")[0].innerHTML = "完成";  //完成按钮
						SetTime(that);
					}
			...
			var that = this;
		 	oSpan3.onclick = function () {
				...
				if (innerText == "&#xe62e;") {  //停止按钮
					...
					this.getElementsByTagName("i")[0].innerHTML = "删除";  //垃圾桶
					clearTimeout(timekeeper);
					that.workLeftTime = that.workTime;
					that.restLeftTime = that.restTime;
					working = false;
